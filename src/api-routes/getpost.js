...
router.get("/api/posts/author/:author", async (req, res) => {
  ...
  try {
    ...
    const query = "SELECT * FROM post JOIN authors ON post.author_id = authors.id JOIN post_category ON post_category.post_id = post.post_id JOIN category ON category.id = post_category.category_id WHERE authors.id = $1"
    const filterPost = await client.query(
      query,
      [author]
    );
    await client.query("COMMIT");
